      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-3">
          {bookmarks.length === 0 ? (
            ...
            bookmarks.map(bm => (
              <div key={bm.id} className={`flex items-center gap-3 p-3 rounded-xl transition-colors border group ${isDarkMode ? 'bg-white/5 border-white/5 hover:bg-white/10' : 'bg-white border-gray-100 hover:bg-gray-50 hover:shadow-sm'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 overflow-hidden ${isDarkMode ? 'bg-white/10' : 'bg-gray-100'}`}>
                  {bm.icon ? (
                ...
                <button
                  ...
                  className="p-1.5 opacity-0 group-hover:opacity-100 text-red-500 hover:bg-red-500/10 rounded-full transition-all"
                  title="Remove"
                >
